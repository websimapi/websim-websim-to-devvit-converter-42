...
(function() {
    ...
    function check(node) {
        ...
        // 1. Detect Raw WebSim URL (missed by interceptor or set via setAttribute)
        if (src.includes('images.websim.ai/avatar/') || src.includes('images.websim.com/avatar/')) {
             try {
                 const parts = src.split('/avatar/');
                 if (parts.length > 1) {
                     const username = parts[1].split(/[?#]/)[0];
                     // Rewrite to placeholder - this will trigger Observer again or we can call resolve directly
                     // We set attribute to ensure consistency and trigger the hook
                     node.src = PLACEHOLDER_PREFIX + username; 
                     return;
                 }
             } catch(e) {}
        }

        // 2. Check for our marker
    ...
    // Hook HTMLImageElement.src to catch dynamic JS assignments
    try {
